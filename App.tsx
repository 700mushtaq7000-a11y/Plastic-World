...
const AppContent: React.FC = () => {
  ...
  const handleQuickBuy = (product: Product) => {
    setQuickBuyItem({ ...product, cartQuantity: 1 });
  ...
  const completeOrder = (customerDetails: { name: string, phone: string, address: string }) => {
    const itemsToOrder = quickBuyItem ? [quickBuyItem] : cart;
    const total = itemsToOrder.reduce((acc, item) => acc + item.price * item.cartQuantity, 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <Navbar 
        ...
        onCartClick={() => { setQuickBuyItem(null); setIsCartOpen(true); }}
        isAdmin={isAdmin}
      ...
      <CartSidebar 
        ...
        onCheckout={() => { setQuickBuyItem(null); setIsCheckoutOpen(true); }}
      />

      {isCheckoutOpen && (
        <CheckoutModal 
          onClose={() => { setIsCheckoutOpen(false); setQuickBuyItem(null); }} 
      ...
      <footer className="bg-slate-900 text-white py-12">
        <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-right">
          ...
          <div>
            ...
            <ul className="space-y-2 text-gray-400">
              <li><button onClick={() => navigate('/')} className="hover:text-blue-400 transition">الرئيسية</button></li>
              <li><a href="#products-section" className="hover:text-blue-400 transition">المنتجات</a></li>
              <li><button onClick={() => navigate('/admin')} className="hover:text-blue-400 transition">لوحة الإدارة</button></li>
            </ul>
          </div>
          <div>
            ...
            <div className="mt-4 flex justify-center md:justify-start gap-4">
               <a href="https://www.facebook.com/share/p/1C5ZtohE5q/" target="_blank" className="bg-[#1877F2] p-2 rounded-full hover:scale-110 transition">
        ...
        <div className="text-center mt-12 text-gray-500 border-t border-slate-800 pt-8">
          جميع الحقوق محفوظة &copy; {new Date().getFullYear()} عالم بلاستك - الكوت
        </div>
      </footer>
    </div>
  );
